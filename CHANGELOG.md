...
- New `url.encode-at` probe in `PrepareCalendar`, recording three subfeatures — one per thing it can actually observe.  It PUTs its own object (`csc_encode_at@example.com`) at the literal-`@` URL rather than going through the normal save path, so the check never depends on which spelling the library happens to mint, and GETs both spellings.  Both resolve → `url.encode-at.literal: full` and `url.encode-at.encoded: full`, and only then is identity observable: a second object (`csc_encode_at_alias@example.com`) is PUT at the `%40` spelling of the same path and the literal one re-read.  Unchanged → `url.encode-at.identity: full`, the two spellings are two resources, which is what RFC3986 §2.2 says they are.  Changed → `identity: unsupported`, the server aliases them; lenient rather than broken, and what every server probed so far actually does.  The second object is written **whether or not `%40` resolved to begin with**, and that is what makes a conformant server observable at all: a server that keeps the two spellings apart answers 404 at `%40` for the simple reason that nothing was ever written there, which looks exactly like a server that cannot serve the encoded spelling — reading the first as the second left `identity` unobserved, so the profile fell back to the library default saying the two spellings are one, on precisely the servers where that is wrong.  `identity` is left `unknown` only when the second write is refused or cannot be completed.  The probe also clears both spellings before it starts: an alias object left behind by an aborted run used to be read as "`%40` does not reach an object stored under a literal `@`", which is false — `%40` had plainly reached an object.  The probe cleans up after itself either way: the second object is deleted where it turned out to be a resource of its own, and the first re-PUT where the server overwrote it.  A 2xx only counts when the body carries one of the probe UIDs, so servers that answer 200 for any child path are not mistaken for support.  Anything undeterminable — a server that serves back neither spelling (a write delay looks exactly like that), a dropped connection, a 5xx on any of the probe's own requests — is recorded `unknown` rather than guessed at, and the probe never raises.  A *refused* write (4xx) is evidence and is recorded as such; a write that could not be completed is not, which matters because "this server refuses a literal `@` in a path" makes the client rewrite every such URL.  The same two questions are asked of **three** things a path can embed an `@` in, because an `.ics` file name is not where this bites: the ownCloud/Nextcloud shape the feature exists for is a calendar-home-set (`/remote.php/dav/calendars/user@example.com/`), an `@` inside a *collection* segment that came from a username, and a server may route that quite differently from an object name.  So there is also a **calendar-id axis** — a probe calendar `csc_encode_at_cal@example.com` is created at the literal spelling, an object without an `@` of its own is stored in it, and that object is asked for through the `%40` spelling of the *collection*: served back means one collection under two names, and where it is not, a second calendar is created at `%40` to tell "a different collection that does not exist yet" from "the encoded spelling does not resolve".  Both probe calendars are deleted again, MKCOL is used where `create-calendar` was probed as `mkcol-required`, and the axis declines to run at all where the server cannot both create and delete calendars.  And a **username axis**: where the username contains an `@` *and* turns up in a path the server itself handed out (the principal URL or the calendar-home-set — an `@` in the authority is userinfo and is left alone), both spellings of that path get a PROPFIND.  It is read-only and yields no identity verdict: establishing identity means writing a second resource at the other spelling, which here would mean creating a second principal, so a breakage on this axis is recorded as "supports the encoded spelling but not the literal one", or the reverse, and nothing is said about identity.
  The three axes share the same three subfeatures rather than getting keys of their own.  Where the axes that observed a subfeature agree, that verdict is recorded; where they disagree — the uid takes both spellings while the calendar path only takes `%40`, say — it is recorded `fragile`, with a `behaviour` naming which axis saw what, since the feature then works in one place and not in another.  An axis that could not answer contributes its `behaviour` text and no vote, so it cannot outvote one that did.  Splitting `url.encode-at.identity` into per-axis children is the right move only once a server is actually seen to differ between them, and none has been yet.  Requires the `url.encode-at` subfeatures in the CalDAV library.
